	...
	if(magic[EI_CLASS] == ELFCLASS32){
		...
		if(opt & OPT_PRO_HEADER){
			...
			for(i=0; i<elfEhdr.e_phnum; i++){
				...
				printf("0x%016jx ", elfPhdr.p_offset);
				printf("0x%016jx ", elfPhdr.p_vaddr);
				printf("0x%016jx ", elfPhdr.p_paddr);
				printf("\n");

				printf("0x%016jx ", elfPhdr.p_filesz);
				printf("0x%016jx ", elfPhdr.p_memsz);
				printf("%-8x ", elfPhdr.p_flags);
				printf("%7jx ", elfPhdr.p_align);
				printf("\n\n");
			}
